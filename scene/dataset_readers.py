...
def readColmapCamerasInterp(cam_extrinsics, cam_intrinsics, images_folder, model_path):
    ...
    for idx, pose_npy in enumerate(pose_interpolated):
        sys.stdout.write("\r")
        sys.stdout.write(
            "Reading camera {}/{}".format(idx + 1, pose_interpolated.shape[0])
        )
        sys.stdout.flush()
...
def readColmapSceneInfo(path, images, eval, args, llffhold=8):
    if eval:
        cameras_extrinsic_file = os.path.join(
            path, f"sparse_{args.n_views}/1", "images.txt"
        )
        cameras_intrinsic_file = os.path.join(
            path, f"sparse_{args.n_views}/1", "cameras.txt"
        )
    else:
        cameras_extrinsic_file = os.path.join(
            path, f"sparse_{args.n_views}/0", "images.txt"
        )
        cameras_intrinsic_file = os.path.join(
            path, f"sparse_{args.n_views}/0", "cameras.txt"
        )
