...
def main(
    ...
    if infer_video:
        ...
        train_img_files, test_img_files = split_train_test(
            image_files, llffhold, n_views, verbose=True
        )
    ...
    loss = scene.compute_global_alignment(
        init="mst", niter=300, schedule=schedule, lr=lr, focal_avg=args.focal_avg
    )
    ...
    if not infer_video:
        ...
        if n_train < n_test:
            ...
            for i in range(n_train - 1):
                tmp_inter_pose = generate_interpolated_path(
                    poses=extrinsics_w2c[i : i + 2], n_interp=n_interp
                )
                all_inter_pose.append(tmp_inter_pose)
            all_inter_pose = np.concatenate(all_inter_pose, axis=0)
            all_inter_pose = np.concatenate(
                [all_inter_pose, extrinsics_w2c[-1][:3, :].reshape(1, 3, 4)], axis=0
            )
        ...
        save_intrinsics(
            sparse_1_path, test_focals, org_imgs_shape, imgs.shape, save_focals=False
        )
    # -----------------------------------------------------------------------------------------
    ...
    print(
        f"[INFO] MASt3R Reconstruction is successfully converted to COLMAP files in: {str(sparse_0_path)}"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--image_size", type=int, default=512, help="Size to resize images"
    )
    parser.add_argument(
        "--schedule", type=str, default="cosine", help="Learning rate schedule"
    )
    parser.add_argument("--lr", type=float, default=0.01, help="Learning rate")
    parser.add_argument("--niter", type=int, default=300, help="Number of iterations")
    parser.add_argument(
        "--min_conf_thr", type=float, default=5, help="Minimum confidence threshold"
    )
