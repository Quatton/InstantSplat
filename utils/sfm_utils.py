...
def save_extrinsic(sparse_path, extrinsics_w2c, img_files, image_suffix):
    ...
    for i, (w2c, img_file) in enumerate(
        zip(extrinsics_w2c, img_files), start=1
...
def save_intrinsics(sparse_path, focals, org_imgs_shape, imgs_shape, save_focals=False):
    ...
    for i, focal in enumerate(focals, start=1):  # Start enumeration from 1
        cameras[i] = Camera(
            ...
            height=org_height,
            params=[
                focal * scale_factor_x,
                focal * scale_factor_y,
                org_width / 2,
                org_height / 2,
            ],
        )
    print(
        f" - scaling focal: ({focal}, {focal}) --> ({focal * scale_factor_x}, {focal * scale_factor_y})"
    )
...
def save_points3D(
    ...
    if use_masks:
        ...
        confs = np.concatenate(
            [p[m] for p, m in zip(confs, masks.reshape(masks.shape[0], -1))]
        )
    else:
        pts = np.array(pts3d)
        col = np.array(imgs)
    ...
    with open(pts_num_file, "a") as f:
        ...
        f.write(
            f"Co_Mask DSP ratio: {co_mask_dsp_pts_num / pts3d.reshape(-1, 3).shape[0]}\n"
        )
        if co_mask_dsp_pts_num > max_pts_num:
            f.write(f"Conf_Mask DSP points num: {conf_dsp_pts_num}\n")
            f.write(
                f"Conf_Mask DSP ratio: {conf_dsp_pts_num / pts3d.reshape(-1, 3).shape[0]}\n"
            )
        f.write("\n")
...
def save_images_and_masks(
    ...
    for i, (image, name, overlapping_mask) in enumerate(
        zip(imgs, image_files, overlapping_masks)
    ):
        ...
        overlapping_mask = (
            np.repeat(np.expand_dims(overlapping_mask, -1), 3, axis=2) * 255
        )
        PIL.Image.fromarray(overlapping_mask.astype(np.uint8)).save(
            overlapping_mask_save_path
        )
...
def cal_co_vis_mask(
    ...
    overlapping_mask[
        y_coords[consistent_depth_mask], x_coords[consistent_depth_mask]
    ] = True
...
def compute_co_vis_masks(
    ...
    for i, curr_map_idx in tqdm(
        enumerate(sorted_conf_indices), total=len(sorted_conf_indices)
    ):
        ...
        before_mask = cal_co_vis_mask(
            points_before,
            depths_before,
            curr_depth_map,
            depth_threshold,
            camera_intrinsics[curr_map_idx],
...
def read_colmap_gt_pose(gt_pose_path, llffhold=8):
    colmap_cam_extrinsics = read_extrinsics_binary(
        gt_pose_path + "/sparse/0/images.bin"
    )
    colmap_cam_extrinsics = {
        k: v
        for k, v in sorted(colmap_cam_extrinsics.items(), key=lambda item: item[1].name)
    }
    all_pose = []
