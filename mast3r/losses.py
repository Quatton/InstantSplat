    ...
    def get_all_pts3d(self, gt1, gt2, pred1, pred2, dist_clip=None):
        ...
        if not self.norm_all:
            if self.max_metric_scale:
                ...
                dist1_to_cam1 = torch.where(
                    valid1, torch.linalg.norm(gt_pts1, dim=-1), 0
                ).view(B, -1)
                dist2_to_cam1 = torch.where(
                    valid2, torch.linalg.norm(gt_pts2, dim=-1), 0
                ).view(B, -1)
        ...
        if self.norm_mode and mask.any():
            pr_pts1[mask], pr_pts2[mask] = normalize_pointcloud(
                pr_pts1[mask], pr_pts2[mask], self.norm_mode, valid1[mask], valid2[mask]
            )
...
class MatchingCriterion(BaseCriterion):
    ...
    def forward(self, a, b, valid_matches=None, euc=False):
        ...
        assert (
            valid_matches is None and dist.ndim == a.ndim - 1
        ) or self.reduction in ["mean", "sum", "1-mean", "none"]
        ...
        if self.reduction == "1-mean":
            return 1.0 - dist.mean() if dist.numel() > 0 else dist.new_ones(())
...
class InfoNCE(MatchingCriterion):
    ...
    def loss(self, desc1, desc2, valid_matches=None, euc=False):
        ...
        if self.mode == "all":  # Previous InfoNCE
            loss = -torch.log(
                (positives / sim.sum(dim=-1).sum(dim=-1, keepdim=True)).clip(self.eps)
            )
        elif self.mode == "proper":  # Proper InfoNCE
            loss = -(
                torch.log((positives / sim.sum(dim=-2)).clip(self.eps))
                + torch.log((positives / sim.sum(dim=-1)).clip(self.eps))
            )
        elif self.mode == "dual":  # Dual Softmax
            loss = -(
                torch.log(
                    (positives**2 / sim.sum(dim=-1) / sim.sum(dim=-2)).clip(self.eps)
                )
            )
        else:
...
class APLoss(MatchingCriterion):
    ...
    def compute_true_AP_torch(scores, labels):
        ...
        with torch.no_grad():
            ...
            recall = torch.arange(1, 1 + npos, dtype=torch.float32, device=dev)[
                None, None, :
            ].expand(B, N, npos)
    ...
    def loss(
        self, desc1, desc2, valid_matches=None, euc=False
    ):  # if matches is None, positives are the diagonal
        B, N1, D = desc1.shape
        B2, N2, D2 = desc2.shape
        assert B == B2 and D == D2
        ...
        labels.diagonal(dim1=-2, dim2=-1)[...] = 1.0
...
class MatchingLoss(Criterion, MultiLoss):
    ...
    def add_negatives(self, outdesc2, desc2, batchid, x2, y2):
        if self.negatives_padding:
            ...
            sel = negatives & (
                negatives.view([B, -1]).cumsum(dim=-1).view(B, H, W)
    ...
    def blockwise_criterion(
        ...
        if (
            N <= self.blocksize
        ):  # Blocks are larger than provided descs, compute regular loss
            ...
            if shuffle:
                matches_perm = np.stack(
                    [rng.choice(range(N), size=N, replace=False) for _ in range(B)]
                )
            ...
            assert N % self.blocksize == 0, (
                "Error, can't chunk block-diagonal, please check blocksize"
            )
            n_chunks = N // self.blocksize
            descs1 = descs1.reshape(
                [B * n_chunks, self.blocksize, D]
            )  # [B*(N//blocksize), blocksize, D]
            descs2 = descs2.reshape(
                [B * n_chunks, self.blocksize, D]
            )  # [B*(N//blocksize), blocksize, D]
            ...
            if self.withconf:
                confs1, confs2 = map(
                    lambda x: x[matches_perm], (confs1, confs2)
                )  # apply perm to confidences if needed

        if self.withconf:
            # split confidences between positives/negatives for loss computation
            details["conf_pos"] = map(
                lambda x: x[valid_matches.view(B, -1)], (confs1, confs2)
            )
            details["conf_neg"] = map(
                lambda x: x[~valid_matches.view(B, -1)], (confs1, confs2)
            )
    ...
    def compute_loss(self, gt1, gt2, pred1, pred2, **kw):
        # Gather preds and GT
        descs1, descs2, confs1, confs2, valid_matches, monitoring = (
            self.get_matching_descs(gt1, gt2, pred1, pred2, **kw)
        )
...
class ConfMatchingLoss(ConfLoss):
    ...
    def __init__(
        self, pixel_loss, alpha=1.0, confmode="prod", neg_conf_loss_quantile=False
    ):
    ...
    def aggregate_confs(
        ...
        if self.confmode == "prod":
            confs = (
                confs1 * confs2 if confs1 is not None and confs2 is not None else 1.0
            )
