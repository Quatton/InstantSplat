...
def postprocess(
    ...
    if desc_dim is not None:
        ...
        if two_confs:
            res["desc_conf"] = reg_dense_conf(
                fmap[..., start + desc_dim], mode=desc_conf_mode
            )
        else:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(patch_size, tuple):
            assert (
                len(patch_size) == 2
                and isinstance(patch_size[0], int)
                and isinstance(patch_size[1], int)
            ), (
                "What is your patchsize format? Expected a single int or a tuple of two ints."
            )
            assert patch_size[0] == patch_size[1], (
                "Error, non square patches not managed"
            )
