...
def matches_to_depths(
    ...
    min_num_valids_ratio=0.3,  # at least this ratio of image pairs need to predict a match for a given pixel of img1
):
    ...
    for b in range(B // batchsize + 1):  # batched processing
        ...
        if sub_batchsize == 0:
            continue
        points1, points2, confs = (
            matches[sub_batch, ..., :2],
            matches[sub_batch, ..., 2:4],
            matches[sub_batch, ..., -1],
        )
        allpoints = torch.cat(
            [
        ...
        assert torch.all(torch.logical_or(validsuni == 0, validsuni == 3)), (
            "Error, can only be nan for none or all XYZ values, not a subset"
        )
        confs[valids_sum == 0] = 0.0
        points_3d_world = points_3d_world * confs[..., None]

        # Take care of NaNs
        normalization = confs.sum(dim=1)[:, None].repeat(1, Nv, 1, 1)
        normalization[normalization <= 1e-5] = 1.0
        points_3d_world[valids] /= (
            normalization[valids_sum == 3][:, None].repeat(1, 3).view(-1)
        )
        points_3d_world[~valids] = 0.0
        aggregated_points = points_3d_world.sum(
            dim=1
        )  # weighted average (by confidence value) ignoring nans
        ...
        points_3d_camera = (
            refcamE[:, :3, :3]
            @ aggregated_points.view(sub_batchsize, -1, 3).transpose(-2, -1)
            + refcamE[:, :3, [3]]
        ).transpose(-2, -1)  # [B,HxW,3]
