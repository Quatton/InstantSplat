...
class SparseGA:
    def __init__(self, img_paths, pairs_in, res_fine, anchors, canonical_paths=None):
        def fetch_img(im):
            def torgb(x):
                return (x[0].permute(1, 2, 0).numpy() * 0.5 + 0.5).clip(
                    min=0.0, max=1.0
                )
    ...
    def get_focals(self):
        return torch.tensor([ff[0, 0] for ff in self.intrinsics]).to(
            self.working_device
        )

    def get_principal_points(self):
        return torch.stack([ff[:2, -1] for ff in self.intrinsics]).to(
            self.working_device
        )
    ...
    def get_dense_pts3d(self, clean_depth=True, subsample=8):
        ...
        for i, canon_path in enumerate(self.canonical_paths):
            ...
            H, W = conf.shape
            pixels = (
                torch.from_numpy(np.mgrid[:W, :H].T.reshape(-1, 2)).float().to(device)
            )
            idxs, offsets = anchor_depth_offsets(
                canon2, {i: (pixels, None)}, subsample=subsample
            )
            anchors[i] = (pixels, idxs[i], offsets[i])

        # densify sparse depthmaps
        pts3d, depthmaps = make_pts3d(
            anchors,
            self.intrinsics,
            self.cam2w,
        ...
        if clean_depth:
            confs = clean_pointcloud(
                confs, self.intrinsics, inv(self.cam2w), depthmaps, pts3d
            )
...
def sparse_scene_optimizer(
    ...
    if shared_intrinsics:
        ...
        confs = torch.stack(
            [torch.load(pth)[0][2].mean() for pth in canonical_paths]
        ).to(pps)
    ...
    min_focals = (
        0.25 * diags
    )  # diag = 1.2~1.4*max(W,H) => beta >= 1/(2*1.2*tan(fov/2)) ~= 0.26
    max_focals = 10 * diags
    ...
    def make_K_cam_depth(log_focals, pps, trans, quats, log_sizes, core_depth):
        ...
        # make extrinsic
        rel_cam2cam = (
            torch.eye(4, dtype=dtype, device=device)[None]
            .expand(len(imgs), 4, 4)
            .clone()
        )
        ...
        cam2w = torch.cat(
            (
                torch.cat((tmp_cam2w[:, :3, :3], new_trans), dim=2),
        ...
        for i in range(len(imgs)):
            ...
            if (
                lora_depth
            ):  # compute core_depth as a low-rank decomposition of 3d points
    ...
    for cci, (img1, pix1, confs, confsum, imgs_slices) in enumerate(corres2d):
        ...
        for img2, slice2 in imgs_slices:
            if is_matching_ok[img1, img2]:
                tslice = slice(
                    curstep, curstep + slice2.stop - slice2.start, slice2.step
                )
...
def make_dense_pts3d(
    intrinsics, cam2w, depthmaps, canonical_paths, subsample, device="cuda"
):
    base_focals = []
    anchors = {}
    confs = []
    ...
    # densify sparse depthmaps
    pts3d, depthmaps_out = make_pts3d(
        anchors,
        intrinsics,
        cam2w,
        [d.ravel() for d in depthmaps],
        base_focals=base_focals,
        ret_depth=True,
    )
    ...
    for img in tqdm(imgs):
        ...
        for (img1, img2), ((path1, path2), path_corres) in tmp_pairs.items():
            ...
            if img == img1:
                ...
                score, (xy1, xy2, confs) = load_corres(
...
def condense_data(imgs, tmp_paths, canonical_views, preds_21, dtype=torch.float32):
    ...
    for idx1, img1 in enumerate(imgs):
        ...
        for img2, (pixels, match_confs) in pixels_confs.items():
            ...
            tmp_pixels[img1, img2] = (
                pixels.to(dtype),
                match_confs.to(dtype),
                slice(*cur_n[-2:]),
            )
        img_anchors[idx1] = (
            torch.cat(img_uv1),
...
def canonical_view(ptmaps11, confs11, subsample, mode="avg-angle"):
    ...
    if mode == "avg-reldepth":
        ...
        stacked_canon = (stacked_confs * rel_depth).sum(dim=0) / stacked_confs.sum(
            dim=0
        )
        ...
        stacked_radius = (stacked_xy.view(B, 2, -1, H, W) - xy[:, :, None, S, S]).norm(
            dim=1
        )
        stacked_radius.clip_(min=1e-8)

        stacked_angle = torch.arctan((stacked_depth - center_depth) / stacked_radius)
        avg_angle = (stacked_confs * stacked_angle).sum(dim=0) / stacked_confs.sum(
            dim=0
        )
...
def backproj(K, depthmap, subsample):
    H, W = depthmap.shape
    uv = np.mgrid[
        subsample // 2 : subsample * W : subsample,
        subsample // 2 : subsample * H : subsample,
    ].T.reshape(H, W, 2)
...
def spectral_projection_depth(
    ...
    lora_proj, coeffs = lora_encode_normed(
...
def spectral_projection_of_depthmaps(
    ...
    for i, img in enumerate(tqdm(imgs)):
        cache = (
            os.path.join(cache_path, "lora_depth", hash_md5(img))
            if cache_path
            else None
        )
        depth, proj = spectral_projection_depth(
...
def bfs(tree, start_node):
    order, predecessors = sp.csgraph.breadth_first_order(
        tree, start_node, directed=False
    )
    ...
    **kw,
    ...
    if gt_cam2w is not None:
        if gt_K is None:
            gt_K = K
        viz.add_cameras(
            to_numpy(gt_cam2w), to_numpy(gt_K), colors=colors, marker="o", **cam_kws
        )
