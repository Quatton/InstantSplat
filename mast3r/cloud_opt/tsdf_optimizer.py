...
class TSDFPostProcess:
    ...
    def __init__(
        self, optimizer, subsample=8, TSDF_thresh=0.0, TSDF_batchsize=int(1e7)
    ):
    ...
    def _get_depthmaps(self, TSDF_filtering_thresh=None):
        if TSDF_filtering_thresh:
            self._refine_depths_with_TSDF(
                self.optimizer, TSDF_filtering_thresh
            )  # compute refined depths if needed
    ...
    def _refine_depths_with_TSDF(self, TSDF_filtering_thresh, niter=1, nsamples=1000):
        ...
        for vi in tqdm(range(self.optimizer.n_imgs)):
            ...
            for it in range(niter):
                ...
                dm_offsets = (
                    torch.randn(H, W, nsamples).to(dm) - 1.0
                ...
                for batch in range(0, len(curproj), self.TSDF_batchsize):
                    values, valid = self._TSDF_query(
                        curproj[batch : min(batch + self.TSDF_batchsize, len(curproj))],
                        curthresh,
                    )
    ...
    def _TSDF_query(self, qpoints, TSDF_filtering_thresh, weighted=True):
        ...
        image_coords = (
            coords_and_depth[..., :2].round().to(int)
        )  # for now, there's no interpolation...
        ...
        all_TSDF_scores = all_SDF_scores.clip(
            -TSDF_filtering_thresh, 1e20
        )  # SDF -> TSDF
    ...
    def _get_pixel_depths(
        self, image_coords, TSDF_filtering_thresh=None, with_normals_conf=False
    ):
        ...
        for ni, (imc, depth, conf) in enumerate(
            ...
            H, W = depth.shape
            valids[ni] = torch.logical_and(
                0 <= imc[:, 1], imc[:, 1] < H
            ) & torch.logical_and(0 <= imc[:, 0], imc[:, 0] < W)
            imc[~valids[ni]] = 0
    ...
    def _pts3d_to_depth(self, pts3d, cam2worlds, focals, pps):
        ...
        # prepare intrinsics
        intrinsics = (
            torch.eye(3).to(cam2worlds)[None].repeat(self.optimizer.n_imgs, 1, 1)
        )
        if len(focals.shape) == 1:
            focals = torch.stack([focals, focals], dim=-1)
        intrinsics[:, 0, 0] = focals[:, 0]
    ...
    def _get_pts3d(self, TSDF_filtering_thresh=None, **kw):
        ...
        """
        return self._backproj_pts3d(
            in_depths=self._get_depthmaps(TSDF_filtering_thresh=TSDF_filtering_thresh),
            **kw,
        )
    ...
    def get_dense_pts3d(self, clean_depth=True):
        if clean_depth:
            confs = clean_pointcloud(
                self.confs,
                self.optimizer.intrinsics,
                inv(self.optimizer.cam2w),
                self.depthmaps,
                self.pts3d,
            )
